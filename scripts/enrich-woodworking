...
async function fetchProducts(asins) {
  ...
  console.log(`Enriching ${asins.length} woodworking ASINs...`);
...
async function run() {
  ...
  console.log(`Woodworking discovery pool: ${discovered.length} total ASINs`);
  console.log(`Woodworking candidates selected: ${candidates.length}`);

  if (!candidates.length) {
    console.log("No woodworking ASINs waiting for enrichment.");
    return;
  }
  ...
  const enrichedSet = new Set(newDeals.map(d => d.asin));
  ...
  console.log(`New valid woodworking deals: ${newDeals.length}`);
